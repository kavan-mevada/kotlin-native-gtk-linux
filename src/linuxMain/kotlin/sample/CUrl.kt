...
fun CPointer<GObject>?.connect(name: String, function: () -> Unit) = this?.reinterpret<GtkWidget>().connect(name, function)

fun GtkStringList(vararg array: String): CPointer<GtkListStore>? {
    val store = gtk_list_store_new (1, G_TYPE_STRING)

    memScoped {
        val iter = alloc<GtkTreeIter>()
        array.forEach {
            gtk_list_store_append (store, iter.ptr)
            gtk_list_store_set (store, iter.ptr, 0, it, -1)
        }
    }
    return store
}